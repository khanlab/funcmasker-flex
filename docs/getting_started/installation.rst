...
Install from github with pip::
    ...
    pip install -e git+https://github.com/akhanf/nnunet_fetal_brain#egg=nnunet_fetal_brain
...
Do a dry-run first (``-n``) and simply print (``-p``) what would be run::
    ...
    nnunet_fetal_brain /path/to/bids/dir /path/to/output/dir participant -np

Run the app, using all cores::
    
    nnunet_fetal_brain /path/to/bids/dir /path/to/output/dir participant --cores all
...
To generate a report, run::
    ...
    nnunet_fetal_brain /path/to/bids/dir /path/to/output/dir participant --report
...
Here are some instructions to get your python environment set-up on graham to run nnunet_fetal_brain:

#. Create a virtualenv and activate it::

      cd $SCRATCH
      module load python/3
      virtualenv venv_nnunet_fetal_brain
      source venv_nnunet_fetal_brain/bin/activate
...
Scaling up to even more subjects (uses group-components to bundle multiple subjects in each job), 1 32core job for N subjects (e.g. 10)::
    ...
    nnunet_fetal_brain bids_dir out_dir participant  --profile cc-slurm --group-components subj=10
